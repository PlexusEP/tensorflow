...
HloInstruction* FindAsyncSendRecvDoneInWhileBody(
  ...
  CHECK_EQ(start_op->user_count(), 1) << "Expect unique user of start op";
  HloInstruction* unique_user = start_op->users().front();
  if (IsSendRecvDone(unique_user)) return unique_user;
  CHECK(unique_user->opcode() == HloOpcode::kTuple && unique_user->IsRoot())
      << "send/recv must be consumed by send/recv-done op or be passed through "
         "the loop.";
...
absl::Status CopyInsertion::AddCopiesForAsyncSendRecv(
  ...
  CHECK_EQ(start_op->users().size(), 1) << "Expect unique user of start";
  HloInstruction* unique_user = start_op->users().front();
  ...
  CHECK_NE(done_op, nullptr) << "Expect send/recv-done in while body.";
